...
router.get('/:pageId', (request, response) => {
    ...
    var minday = new Date('2022-08-24');
    var inputday = new Date(pageID);

    console.log(minday);
    console.log(inputday);


    if (inputday<minday) {  // 로그인 안되어있으면 로그인 페이지로 이동시킴
        response.redirect('/auth/login');
        return false;
    }

    db.query(`UPDATE reserveTable SET t10=? WHERE date=?`, ["홍길동", pageID],function (error, result) {
        if (error) throw error;
        //response.redirect(`/page/${post.id}`);
      });
      ...
      db.query('SELECT * FROM reserveTable WHERE date = ?', [pageID], function (error2, results) {
          //console.log(results);
          //console.log(results[0].t2);
          for (const [key, value] of Object.entries(results[0])) {
              if (key !== "id" && key !== "date" && key !== "disable") {
          ...
          var html = template.HTML(title,'<script src="script_admin2.js"></script>',        
          `
          <h3 style="text-align: center;">남도레미 악기연습실 예약 시스템 - admin</h3>
              <div class="accountBox"> 
                  <span><b>기획팀 홍길동</b>님 환영합니다</span>
